# Backend — FastAPI (SQLModel + libSQL)
...
- `provider_uri` defaults to `backend/qlib_data` (`QLIB_PROVIDER_URI` env overrides). `ensure_initialized()` calls `qlib.init(provider_uri, region="in")` once and seeds `calendars/day.txt` with the last 30 trading days if absent.
